...
import com.example.foodsaverapps.ChangePassword2Activity
import com.example.foodsaverapps.Model.UserModel
import com.example.foodsaverapps.ProfileInformation2Activity
...
class Profile2Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.imageButtonProfileInformation.setOnClickListener {
            val intent = Intent(requireContext(), ProfileInformation2Activity::class.java)
            startActivityForResult(intent, PROFILE_INFORMATION_REQUEST_CODE)
        }

        binding.imageButtonChangePassword.setOnClickListener {
            val intent = Intent(requireContext(), ChangePassword2Activity::class.java)
    ...
    private fun updateProfilePictureUri(uri: String) {
        val userId = auth.currentUser?.uid ?: return
        val userRef = database.getReference("seller").child(userId)
    ...
    companion object {
        private const val PROFILE_INFORMATION_REQUEST_CODE = 200
    }
}
